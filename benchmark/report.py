...
def print_benchmark_report(
    ...
    for opt in config.options:
        ...
        print(f"\n{' ' * 20}{'-' * 40}")
        print(
            f"{' ' * 23}OPTION: {opt_label} | K={opt.strike:.1f} | T={opt.maturity:.3f}"
        )
        print(f"{' ' * 20}{'-' * 40}\n")
        ...
        header = f"{'Metric':<12}" + "".join(
            [f"{name:^{col_width}}" for name in technique_names]
        )
        print(header)
        print("-" * len(header))
    ...
    if parity_is_possible:
        ...
        parity_header = f"{'Technique':<12}" + "".join(
            [f"{name:^{col_width}}" for name in technique_names]
        )
        print(parity_header)
