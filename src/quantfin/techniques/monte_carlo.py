...
class MonteCarloTechnique(BaseTechnique, GreekMixin, IVMixin):
    ...
    """
    def __init__(self, *, n_paths: int = 20_000, n_steps: int = 100, antithetic: bool = True, seed: int | None = None):
    ...
    def price(self, option: Option, stock: Stock, model: BaseModel, rate: Rate, **kwargs: Any) -> PricingResult:
        ...
        payoff = np.maximum(ST - K, 0) if option.option_type is OptionType.CALL else np.maximum(K - ST, 0)
        price = float(np.mean(payoff) * math.exp(-r * T))
        return PricingResult(price=price)

    def _get_sde_kernel_and_params(self, model: BaseModel, r: float, q: float, dt: float, **kwargs) -> tuple[Callable, Dict[str, Any]]:
        ...
        if model.has_variance_process and model.has_jumps: # Bates
            kernel_params.update({'kappa': p['kappa'], 'theta': p['theta'], 'rho': p['rho'], 'vol_of_vol': p['vol_of_vol'], 'v0': kwargs.get('v0', p.get('v0'))})
            kernel_params.update({'lambda_': p['lambda'], 'mu_j': p['mu_j'], 'sigma_j': p['sigma_j']})
            ...
            kernel_params.update({'sigma': p['sigma'], 'lambda_': p['lambda'], 'p_up': p['p_up'], 'eta1': p['eta1'], 'eta2': p['eta2']})
            return kou_kernel, kernel_params
        elif model.has_jumps: # Merton
            kernel_params.update({'sigma': p['sigma'], 'lambda_': p['lambda'], 'mu_j': p['mu_j'], 'sigma_j': p['sigma_j']})
    ...
    def _simulate_sde_path(self, model: BaseModel, S0: float, r: float, q: float, T: float, **kwargs) -> np.ndarray:
        ...
        if model.has_variance_process:
            dw_v = self.rng.standard_normal(size=(num_draws, self.n_steps)) * math.sqrt(dt)
            dw_uncorr = self.rng.standard_normal(size=(num_draws, self.n_steps)) * math.sqrt(dt)
            rho = model.params.get('rho', 0)
            sim_params['dw1'] = rho * dw_v + math.sqrt(1 - rho**2) * dw_uncorr
            sim_params['dw2'] = dw_v
        else:
            sim_params['dw'] = self.rng.standard_normal(size=(num_draws, self.n_steps)) * math.sqrt(dt)
        ...
        if self.antithetic and not getattr(model, 'is_local_vol', False):
            ...
            if 'dw2' in sim_params: sim_params['dw2'] *= -1
            ST_anti = kernel(**sim_params)
            ST = np.concatenate([ST, ST_anti])

        return ST if getattr(model, 'is_sabr', False) else np.exp(ST)

    def _simulate_levy_terminal(self, model: BaseModel, S0: float, r: float, q: float, T: float) -> np.ndarray:
