...
class LatticeTechnique(BaseTechnique, GreekMixin, IVMixin):
    ...
    def delta(self, option: Option, stock: Stock, model: BaseModel, rate: Rate, **kwargs: Any) -> float:
        ...
        if not self._cached_nodes: self.price(option, stock, model, rate)
        cache = self._cached_nodes
        return (cache['price_up'] - cache['price_down']) / (cache['spot_up'] - cache['spot_down'])
    ...
    @abstractmethod
    def _price_and_get_nodes(self, option: Option, stock: Stock, model: BaseModel, rate: Rate) -> dict[str, Any]:
...
class CRRTechnique(LatticeTechnique):
    ...
    def _price_and_get_nodes(self, option: Option, stock: Stock, model: BaseModel, rate: Rate) -> dict[str, Any]:
        ...
        return _crr_pricer(S0=stock.spot, K=option.strike, T=option.maturity, r=rate.get_rate(option.maturity), q=stock.dividend, sigma=sigma, N=self.steps, is_call=(option.option_type is OptionType.CALL), is_am=self.is_american)

class LeisenReimerTechnique(LatticeTechnique):
    """Leisen-Reimer binomial lattice technique with Peizer-Pratt inversion."""
    def _price_and_get_nodes(self, option: Option, stock: Stock, model: BaseModel, rate: Rate) -> dict[str, Any]:
        sigma = model.params.get("sigma", stock.volatility)
        return _lr_pricer(S0=stock.spot, K=option.strike, T=option.maturity, r=rate.get_rate(option.maturity), q=stock.dividend, sigma=sigma, N=self.steps, is_call=(option.option_type is OptionType.CALL), is_am=self.is_american)

class TOPMTechnique(LatticeTechnique):
    """Kamrad-Ritchken trinomial lattice technique."""
    def _price_and_get_nodes(self, option: Option, stock: Stock, model: BaseModel, rate: Rate) -> dict[str, Any]:
        sigma = model.params.get("sigma", stock.volatility)
        return _topm_pricer(S0=stock.spot, K=option.strike, T=option.maturity, r=rate.get_rate(option.maturity), q=stock.dividend, vol=sigma, N=self.steps, is_call=(option.option_type is OptionType.CALL), is_am=self.is_american)

def _crr_pricer(S0: float, K: float, T: float, r: float, q: float, sigma: float, N: int, is_call: bool, is_am: bool) -> dict[str, Any]:
    ...
    j = np.arange(N + 1)
    payoff = np.where(is_call, np.maximum(S0 * u**j * d**(N - j) - K, 0.0), np.maximum(K - S0 * u**j * d**(N - j), 0.0))
    for i in range(N - 1, -1, -1):
        ...
        if is_am:
            ...
            early_exercise = np.where(is_call, np.maximum(stock_prices - K, 0.0), np.maximum(K - stock_prices, 0.0))
            payoff = np.maximum(payoff, early_exercise)
    return {"price": payoff[0], "price_up": price_up, "price_down": price_down, "price_uu": price_uu, "price_ud": price_ud, "price_dd": price_dd, "spot_up": S0 * u, "spot_down": S0 * d, "spot_uu": S0 * u * u, "spot_ud": S0 * u * d, "spot_dd": S0 * d * d}

def _lr_pricer(S0: float, K: float, T: float, r: float, q: float, sigma: float, N: int, is_call: bool, is_am: bool) -> dict[str, Any]:
    if sigma < 1e-6:
        price = max(0.0, S0 - K) if is_call else max(0.0, K - S0)
        return {"price": price, "price_up": price, "price_down": price, "price_uu": price, "price_ud": price, "price_dd": price, "spot_up": S0, "spot_down": S0, "spot_uu": S0, "spot_ud": S0, "spot_dd": S0}
    if N % 2 == 0: N += 1
    dt = T / N
    disc = math.exp(-r * dt)
...
def _peizer_pratt(z: float, N: int) -> float:
    if abs(z) > 50: return 1.0 if z > 0 else 0.0
    term = z / (N + 1/3 + 0.1 / (N + 1))
    return 0.5 + math.copysign(0.5 * math.sqrt(1 - math.exp(-term**2 * (N + 1/6))), z)

def _topm_pricer(S0: float, K: float, T: float, r: float, q: float, vol: float, N: int, is_call: bool, is_am: bool) -> dict[str, Any]:
    ...
    pm = 1.0 - pu - pd
    j = np.arange(-N, N + 1)
    payoff = np.where(is_call, np.maximum(S0 * np.exp(j * dx) - K, 0.0), np.maximum(K - S0 * np.exp(j * dx), 0.0))
    for i in range(N - 1, -1, -1):
        if i == 0:
        ...
        if is_am:
            ...
            early_exercise = np.where(is_call, np.maximum(stock_prices - K, 0.0), np.maximum(K - stock_prices, 0.0))
            payoff = np.maximum(payoff, early_exercise)
    return {"price": payoff[0], "price_up": price_up, "price_mid": price_mid, "price_down": price_down, "spot_up": S0 * math.exp(dx), "spot_mid": S0, "spot_down": S0 * math.exp(-dx)}