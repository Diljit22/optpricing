...
class GreekMixin:
    ...
    """
    def delta(self, option: Option, stock: Stock, model: BaseModel, rate: Rate, h_frac: float = 1e-3, **kwargs: Any) -> float:
        h = stock.spot * h_frac
        ...
        if isinstance(rng, np.random.Generator):
            ...
            with crn(rng): p_dn = self.price(option, stock_dn, model, rate, **kwargs).price
        else:
    ...
    def gamma(self, option: Option, stock: Stock, model: BaseModel, rate: Rate, h_frac: float = 1e-3, **kw: Any) -> float:
        ...
        rng = getattr(self, "rng", None)
        if isinstance(rng, np.random.Generator):
            with crn(rng): p_up = self.price(option, stock_up, model, rate, **kw).price
            with crn(rng): p_0 = self.price(option, stock, model, rate, **kw).price
            with crn(rng): p_dn = self.price(option, stock_dn, model, rate, **kw).price
        else:
    ...
    def vega(self, option: Option, stock: Stock, model: BaseModel, rate: Rate, h: float = 1e-4, **kw: Any) -> float:
        ...
        rng = getattr(self, "rng", None)
        if isinstance(rng, np.random.Generator):
            with crn(rng): p_up = self.price(option, stock, model_up, rate, **kw).price
            with crn(rng): p_dn = self.price(option, stock, model_dn, rate, **kw).price
        else:
    ...
    def theta(self, option: Option, stock: Stock, model: BaseModel, rate: Rate, h: float = 1e-5, **kw: Any) -> float:
        T0 = option.maturity
        ...
        rng = getattr(self, "rng", None)
        if isinstance(rng, np.random.Generator):
            with crn(rng): p_up = self.price(opt_up, stock, model, rate, **kw).price
            with crn(rng): p_dn = self.price(opt_dn, stock, model, rate, **kw).price
        else:
    ...
    def rho(self, option: Option, stock: Stock, model: BaseModel, rate: Rate, h: float = 1e-4, **kw: Any) -> float:
        ...
        rng = getattr(self, "rng", None)
        if isinstance(rng, np.random.Generator):
            with crn(rng): p_up = self.price(option, stock, model, rate_up, **kw).price
            with crn(rng): p_dn = self.price(option, stock, model, rate_dn, **kw).price
        else:
