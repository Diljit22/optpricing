...
class TOPMLatticeTechnique(BaseTechnique, GreekMixin, IVMixin, ABC):
    ...
    def delta(
        ...
        V_up = _topm_price(
            S0 * B, K, T, r, q, vol, self.steps, is_call, self.is_american
        )
        V_dn = _topm_price(
            S0 / B, K, T, r, q, vol, self.steps, is_call, self.is_american
        )
    ...
    def gamma(
        ...
        V_mid = _topm_price(S0, K, T, r, q, vol, self.steps, is_call, self.is_american)
        V_dn = _topm_price(
            S0 / B, K, T, r, q, vol, self.steps, is_call, self.is_american
        )
        ...
        return 2.0 * (h2 * V_up - (h1 + h2) * V_mid + h1 * V_dn) / (h1 * h2 * (h1 + h2))
...
def _topm_price(
    ...
    for i in range(N - 1, -1, -1):
        M = 2 * i + 1
        pay[:M] = disc * (pU * pay[2 : M + 2] + pS * pay[1 : M + 1] + pD * pay[:M])
        if is_am:
            idx = N - i
            cont = np.where(
                is_call,
