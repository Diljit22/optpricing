...
def _fetch_from_polygon(ticker: str) -> pd.DataFrame | None:
    ...
    if not POLYGON_API_KEY or "YOUR_KEY" in POLYGON_API_KEY:
        print("Error: Polygon API key not configured in config.yaml or environment variables.")
        return None
    ...
    try:
        ...
        for contract_details in snapshot.results:
            ...
            data.append({
                ...
                'optionType': 'call' if contract_details.details.contract_type == 'call' else 'put',
...
def get_live_option_chain(ticker: str) -> pd.DataFrame | None:
    ...
    if provider == "polygon":
        ...
        print(f"Warning: Unknown live_data_provider '{provider}'. Defaulting to yfinance.")
        return _fetch_from_yfinance(ticker)
...
def get_available_snapshot_dates(ticker: str) -> List[str]:
    ...
    try:
        files = [f.name for f in MARKET_SNAPSHOT_DIR.iterdir() if f.name.startswith(f"{ticker}_") and f.name.endswith(".parquet")]
        return sorted([f.replace(f"{ticker}_", "").replace(".parquet", "") for f in files], reverse=True)
